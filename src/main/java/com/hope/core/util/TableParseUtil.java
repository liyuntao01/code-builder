...
public class TableParseUtil {
    ...
    public static ClassInfo processTableIntoClassInfo(String tableSql, CodeConfig codeConfig) throws IOException {
        ...
        String[] prefix = codeConfig.getTbPrefix().split(",");

        for (String str : prefix) {
            tableName = tableName.replaceFirst(str, "");
        }

        //slashName
        String slashName = tableName.replaceAll("_", "-");

        //variableName
        String variableName = StringUtils.underlineToCamelCase(tableName);
        ...
        for (Pattern pattern : Arrays.asList(
            ...
            while (patternMatcher.find()) {
                String target = patternMatcher.group();

                fieldListTmp = fieldListTmp.replace(target, "");
            }
        }
        ...
        if (fieldLineList.length > 0) {
            for (String columnLine : fieldLineList) {
                ...
                if (columnLine.startsWith("`")) {
                    ...
                    if (columnLine.startsWith("int") || columnLine.startsWith("smallint")) {
                        fieldClass = "Integer";
                    ...
                    } else if (columnLine.startsWith("tinyint(1)")) {
                        fieldClass = "Integer";
                    } else if (columnLine.startsWith("tinyint")) {
                        fieldClass = "Integer";
                    }
                    ...
                    if (fieldComment.contains("唯一")) {
                        checkFields.put(fieldName, fieldComment.replace("唯一", ""));
                        fieldInfo.setFieldComment(fieldComment.replace("唯一", ""));
                    }
                    if (fieldName.contains("Id") && !fieldName.contains("ById")
                            && !fieldName.contains("tenantId")
                            && codeJavaInfo.getAttached() == null
                            && !fieldComment.contains("非")) {
                        codeJavaInfo.setAttached(fieldName);
                        codeJavaInfo.setAttachedPrefix(fieldComment);
                    }
                    fieldInfo.setFieldComment(fieldComment.replace("非", ""));
        ...
        codeJavaInfo.setTableName(originTableName);
        codeJavaInfo.setClassName(className);
        codeJavaInfo.setVariableName(variableName);
        codeJavaInfo.setSlashName(slashName);
        codeJavaInfo.setClassComment(classComment);
        codeJavaInfo.setFieldList(fieldList);
        codeJavaInfo.setCheckFields(checkFields);
